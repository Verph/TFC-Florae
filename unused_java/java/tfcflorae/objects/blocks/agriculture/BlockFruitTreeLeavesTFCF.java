...
public class BlockFruitTreeLeavesTFCF extends BlockLeaves
{
    public static final PropertyEnum<EnumLeafState> LEAF_STATE = PropertyEnum.create("state", BlockFruitTreeLeavesTFCF.EnumLeafState.class);
    public static final PropertyBool HARVESTABLE = PropertyBool.create("harvestable");
    private static final Map<IFruitTree, BlockFruitTreeLeavesTFCF> MAP = new HashMap<>();

    public static BlockFruitTreeLeavesTFCF get(IFruitTree tree)
    {
        return MAP.get(tree);
    }
    ...
    public BlockFruitTreeLeavesTFCF(IFruitTree tree)
    {
        this.tree = tree;
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            for (BlockPos p1 : paths)
            {
                for (EnumFacing face : EnumFacing.values())
                {
                    ...
                    if (state1.getBlock() == BlockFruitTreeTrunkTFCF.get(tree) || state1.getBlock() == BlockFruitTreeBranchTFCF.get(tree))
                        return;
