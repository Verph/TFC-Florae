...
public class BlockFruitTreeSaplingTFCF extends BlockBush implements IGrowable
{
    private static final AxisAlignedBB SAPLING_AABB = new AxisAlignedBB(0.1, 0, 0.1, 0.9, 0.9, 0.9);

    private static final Map<IFruitTree, BlockFruitTreeSaplingTFCF> MAP = new HashMap<>();

    public static BlockFruitTreeSaplingTFCF get(IFruitTree tree)
    {
        return MAP.get(tree);
    }
    ...
    public BlockFruitTreeSaplingTFCF(IFruitTree tree)
    {
    ...
    public void grow(World world, Random random, BlockPos blockPos, IBlockState blockState)
    {
        if (!world.isRemote)
        {
            world.setBlockState(blockPos, BlockFruitTreeTrunkTFCF.get(this.tree).getDefaultState());
            if (world.getBlockState(blockPos.up()).getMaterial().isReplaceable())
            {
                world.setBlockState(blockPos.up(), BlockFruitTreeLeavesTFCF.get(tree).getDefaultState().withProperty(BlockFruitTreeLeavesTFCF.HARVESTABLE, false));
            }
        }
    }
