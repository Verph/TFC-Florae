...
public enum BarrierReefOceanBorderLayer implements AdjacentTransformLayer
{
    ...
    public int apply(AreaContext context, int north, int east, int south, int west, int center)
    {
        ...
        if (center == BARRIER_REEF_SHORE_MARKER || center == BARRIER_REEF_OCEAN_MARKER)
        {
            if (matcher.test(i -> i != BARRIER_REEF_OCEAN_MARKER && i != BARRIER_REEF_SHORE_MARKER && !(i == BARRIER_REEF_MARKER || i == BARRIER_REEF)))
            {
