...
public class DripstoneClusterFeature extends Feature<DripstoneClusterConfiguration>
{
   ...
   public boolean place(FeaturePlaceContext<DripstoneClusterConfiguration> context)
   {
      ...
      final DripstoneClusterConfiguration config = context.config();
      ...
      else
      {
         ...
         for(int l = -j; l <= j; ++l)
         {
            for(int i1 = -k; i1 <= k; ++i1)
            {
               ...
               this.placeColumn(level, random, pos1, l, i1, f, d0, i, f1, config);
            }
         }
         return true;
      }
   ...
   private void placeColumn(WorldGenLevel level, Random random, BlockPos pos, int x, int z, float wetness, double chance, int height, float density, DripstoneClusterConfiguration config) {
      ...
      if (optional.isPresent()) {
         ...
         if (optionalint.isPresent() || optionalint1.isPresent()) {
            ...
            if (optionalint.isPresent()) {
               DripstoneUtils.growPointedDripstone(level, pos.atY(optionalint.getAsInt() - 1), Direction.DOWN, j3, flag3);
            }

            if (optionalint2.isPresent()) {
               DripstoneUtils.growPointedDripstone(level, pos.atY(optionalint2.getAsInt() + 1), Direction.UP, j1, flag3);
            }
   ...
   private int getDripstoneHeight(Random random, int x, int z, float chance, int height, DripstoneClusterConfiguration config) {
   ...
   private boolean canPlacePool(WorldGenLevel level, BlockPos pos) {
      ...
      if (!blockstate.is(Blocks.WATER) && !blockstate.is(TFCFBlocks.DRIPSTONE_BLOCKS.get(DripstoneRock.DRIPSTONE).get(Rock.BlockType.RAW).get()) && !blockstate.is(TFCFBlocks.DRIPSTONE_BLOCKS.get(DripstoneRock.DRIPSTONE).get(Rock.BlockType.SPIKE).get())) {
   ...
   private void replaceBlocksWithDripstoneBlocks(WorldGenLevel level, BlockPos pos, int pThickness, Direction pDirection) {
      BlockPos.MutableBlockPos pos$mutablepos = pos.mutable();

      for(int i = 0; i < pThickness; ++i) {
         if (!DripstoneUtils.placeDripstoneBlockIfPossible(level, pos$mutablepos)) {
            return;
         }
