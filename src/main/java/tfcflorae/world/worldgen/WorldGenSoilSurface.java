...
public class WorldGenSoilSurface implements IWorldGenerator
{
    ...
    public void generate(Random random, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator, IChunkProvider chunkProvider)
    {
        ...
        if (ConfigTFCF.General.WORLD.enableAllBlockTypes)
        {
            ...
            {
                BlockPos pos = world.getTopSolidOrLiquidBlock(chunkBlockPos.add(8 + random.nextInt(16), 16, 8 + random.nextInt(16)));
                generateCoarseSoilSurfaceNew(world, random, pos);
            }

            /*
            pos = world.getTopSolidOrLiquidBlock(chunkBlockPos.add(8 + random.nextInt(16), 16, 8 + random.nextInt(16)));
                    ...
                    for (int width = -tz; width <= tz; width++)
                    {
                        ...
                        for (int y = -depth; y <= +depth; y++)
                        {
                            ...
                            if (normalRarity == 0)
                            {
                                ...
                                else if ((BlocksTFC.isDryGrass(current)) && ConfigTFCF.General.WORLD.enableAllSparseGrass)
                                {
                                ...
                                else if ((BlocksTFC.isGrass(current)) && ConfigTFCF.General.WORLD.enableAllSparseGrass)
                                {
                                    world.setBlockState(pos, BlockRockVariantTFCF.get(ChunkDataTFC.getRockHeight(world, pos), RockTFCF.SPARSE_GRASS).getDefaultState(), 2);
                                }
                            }
    ...
    private void generateCoarseSoilSurface(World world, Random rng, BlockPos start)
    {
        ...
        {
            ChunkDataTFC data = ChunkDataTFC.get(world, start);
            if (data.isInitialized() && data.getFloraDensity() >= 0.4f && (data.getAverageTemp() >= 15f || data.getAverageTemp() <= 5))
            {
                int length = rng.nextInt(4) + 3;
                int depth = rng.nextInt(3) + 1;
                float widthMultiplier = rng.nextInt(1) + 1f;
