...
public class BlockRockVariantFallableTFCF extends BlockRockVariantTFCF implements IFallingBlock
{
    ...
    {
        if (!this.rockTFCF.canFall()) return;
    ...
    {
        if (rockTFCF.canFall() && shouldFall(world, pos, pos, ignoreSupportChecks))
        {
            return checkAreaClear(world, pos);
        }
        if (rockTFCF.canFallHorizontal())
        {
