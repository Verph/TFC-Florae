...
public class BlockHangingPlantTFCF extends BlockCreepingPlantTFCF implements IGrowable
{
    private static final PropertyBool BOTTOM = PropertyBool.create("bottom");
    private static final Map<PlantTFCF, BlockHangingPlantTFCF> MAP = new HashMap<>();

    public static BlockHangingPlantTFCF get(PlantTFCF plantTFCF)
    {
        return BlockHangingPlantTFCF.MAP.get(plantTFCF);
    }

    public BlockHangingPlantTFCF(PlantTFCF plantTFCF)
    {
        super(plantTFCF);
    ...
    public boolean canGrow(World worldIn, BlockPos pos, IBlockState state, boolean isClient)
    {
        ...
        return i < plantTFCF.getMaxHeight() && worldIn.isAirBlock(pos.down()) && ((!material.isSolid() || material == Material.LEAVES)) && canBlockStay(worldIn, pos.down(), state);
    }
    ...
    public void grow(World worldIn, Random rand, BlockPos pos, IBlockState state)
    {
        ...
        IBlockState iblockstate = state.withProperty(AGE, 0).withProperty(growthStageProperty, plantTFCF.getStageForMonth()).withProperty(BOTTOM, false);
    ...
    protected boolean canConnectTo(IBlockAccess worldIn, BlockPos pos, EnumFacing facing)
    {
        ...
        return material == Material.CLAY || material == Material.CLOTH || material == Material.GOURD || material == Material.GROUND || material == Material.ICE || material == Material.IRON || material == Material.LEAVES || material == Material.PACKED_ICE || material == Material.ROCK || material == Material.SAND || material == Material.SNOW || material == Material.SPONGE || material == Material.WOOD;
    }
        ...
        if (!state.getValue(BOTTOM))
        {
            for (EnumFacing face : EnumFacing.Plane.HORIZONTAL.facings())
            {
                ...
                if (rand.nextDouble() < 0.5D && worldIn.isAirBlock(sidePos) && worldIn.isAirBlock(sidePos.down()))
                {
                    ...
                    IBlockState iblockstate = state.withProperty(AGE, 0).withProperty(growthStageProperty, plantTFCF.getStageForMonth());
                    worldIn.setBlockState(pos, iblockstate);
