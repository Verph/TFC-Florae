...
public class BlockCreepingPlantTFCF extends BlockPlantTFCF
{
    ...
    private static final Map<PlantTFCF, BlockCreepingPlantTFCF> MAP = new HashMap<>();

    public static BlockCreepingPlantTFCF get(PlantTFCF plantTFCF)
    {
        return BlockCreepingPlantTFCF.MAP.get(plantTFCF);
    }

    public BlockCreepingPlantTFCF(PlantTFCF plantTFCF)
    {
        super(plantTFCF);
    ...
    public boolean canBlockStay(World worldIn, BlockPos pos, IBlockState state)
    {
        for (EnumFacing face : EnumFacing.values())
        {
            ...
            {
                return plantTFCF.isValidTemp(ClimateTFC.getActualTemp(worldIn, pos)) && plantTFCF.isValidRain(ChunkDataTFC.getRainfall(worldIn, pos));
            }
        }
        return false;
    }
    ...
    public IBlockState withRotation(IBlockState state, Rotation rot)
    {
        ...
        {
            case CLOCKWISE_180:
                return state.withProperty(growthStageProperty, plantTFCF.getStageForMonth()).withProperty(NORTH, state.getValue(SOUTH)).withProperty(EAST, state.getValue(WEST)).withProperty(SOUTH, state.getValue(NORTH)).withProperty(WEST, state.getValue(EAST));
            case COUNTERCLOCKWISE_90:
                return state.withProperty(growthStageProperty, plantTFCF.getStageForMonth()).withProperty(NORTH, state.getValue(EAST)).withProperty(EAST, state.getValue(SOUTH)).withProperty(SOUTH, state.getValue(WEST)).withProperty(WEST, state.getValue(NORTH));
            case CLOCKWISE_90:
                return state.withProperty(growthStageProperty, plantTFCF.getStageForMonth()).withProperty(NORTH, state.getValue(WEST)).withProperty(EAST, state.getValue(NORTH)).withProperty(SOUTH, state.getValue(EAST)).withProperty(WEST, state.getValue(SOUTH));
            default:
                return state.withProperty(growthStageProperty, plantTFCF.getStageForMonth());
        }
    }
    ...
    public IBlockState withMirror(IBlockState state, Mirror mirrorIn)
    {
        ...
        {
            case LEFT_RIGHT:
                return state.withProperty(growthStageProperty, plantTFCF.getStageForMonth()).withProperty(NORTH, state.getValue(SOUTH)).withProperty(SOUTH, state.getValue(NORTH));
            case FRONT_BACK:
                return state.withProperty(growthStageProperty, plantTFCF.getStageForMonth()).withProperty(EAST, state.getValue(WEST)).withProperty(WEST, state.getValue(EAST));
            default:
