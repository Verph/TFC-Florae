...
public class BlockPlantTFCF extends BlockBush implements IItemSize
{
    ...
    public static BlockPlantTFCF get(PlantTFCF plant)
    {
    ...
    protected final PlantTFCF plant;
    protected final BlockStateContainer blockState;

    public BlockPlantTFCF(PlantTFCF plant)
    {
    ...
    {
        if (!plant.getOreDictName().isPresent() && !worldIn.isRemote && (stack.getItem().getHarvestLevel(stack, "knife", player, state) != -1 || stack.getItem().getHarvestLevel(stack, "scythe", player, state) != -1))
        {
    ...
    @Nonnull
    public PlantTFCF.EnumPlantTypeTFCF getPlantTypeTFC()
    {
    ...
    private boolean isValidSoil(IBlockState state)
    {
        switch (plant.getPlantType())
        {
            ...
            case EMERGENT_TALL_WATER:
                return BlocksTFC.isSand(state) || BlocksTFC.isSoilOrGravel(state) || BlocksTFCF.isSand(state) || BlocksTFCF.isSoilOrGravel(state);
            case WATER_SEA:
            case TALL_WATER_SEA:
