...
import tfcflorae.world.surface.builder.ForestSurfaceBuilder;
...
public final class TFCBiomesMixin
{
    ...
    // Low biomes
    @Shadow @Mutable @Final
    public static final BiomeExtension PLAINS = register("plains", builder().heightmap(seed -> BiomeNoise.hills(seed, 4, 10)).surface(ForestSurfaceBuilder.create(NormalSurfaceBuilder.INSTANCE)).spawnable()); // Very flat, slightly above sea level.
    @Shadow @Mutable @Final
    public static final BiomeExtension HILLS = register("hills", builder().heightmap(seed -> BiomeNoise.hills(seed, -5, 16)).surface(ForestSurfaceBuilder.create(NormalSurfaceBuilder.INSTANCE)).spawnable()); // Small hills, slightly above sea level.
    @Shadow @Mutable @Final
    public static final BiomeExtension LOWLANDS = register("lowlands", builder().heightmap(BiomeNoise::lowlands).surface(ForestSurfaceBuilder.create(LowlandsSurfaceBuilder.INSTANCE)).aquiferHeightOffset(-16).spawnable()); // Flat, swamp-like, lots of shallow pools below sea level.
    @Shadow @Mutable @Final
    public static final BiomeExtension LOW_CANYONS = register("low_canyons", builder().heightmap(seed -> BiomeNoise.canyons(seed, -8, 21)).surface(ForestSurfaceBuilder.create(NormalSurfaceBuilder.INSTANCE)).aquiferHeightOffset(-16).spawnable()); // Sharp, small hills, with lots of water / snaking winding rivers.
