...
import tfcflorae.client.screen.button.EarthenwareVesselSealButton;
...
public class LargeEarthenwareVesselScreen extends BlockEntityScreen<LargeEarthenwareVesselBlockEntity, LargeEarthenwareVesselContainer>
{
    ...
    public LargeEarthenwareVesselScreen(LargeEarthenwareVesselContainer container, Inventory playerInventory, Component name)
    {
    ...
    {
        super.init();
        addRenderableWidget(new EarthenwareVesselSealButton(blockEntity, getGuiLeft() + 9, getGuiTop(), new Button.OnTooltip()
        {
            @Override
    ...
    protected void renderLabels(PoseStack poseStack, int mouseX, int mouseY)
    {
        ...
        if (isSealed())
        {
            drawDisabled(poseStack, 0, LargeEarthenwareVesselBlockEntity.SLOTS - 1);
        }
    }

    private boolean isSealed()
    {
        return blockEntity.getBlockState().getValue(LargeEarthenwareVesselBlock.SEALED);
    }
}
