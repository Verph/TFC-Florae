...
public class BlockFruitTreeSaplingTFCE extends BlockBush implements IGrowable
{
    private static final AxisAlignedBB SAPLING_AABB = new AxisAlignedBB(0.1, 0, 0.1, 0.9, 0.9, 0.9);

    private static final Map<IFruitTree, BlockFruitTreeSaplingTFCE> MAP = new HashMap<>();

    public static BlockFruitTreeSaplingTFCE get(IFruitTree tree)
    {
        return MAP.get(tree);
    }
    ...
    public BlockFruitTreeSaplingTFCE(IFruitTree tree)
    {
    ...
    public void grow(World world, Random random, BlockPos blockPos, IBlockState blockState)
    {
        if (!world.isRemote)
        {
            world.setBlockState(blockPos, BlockFruitTreeTrunkTFCE.get(this.tree).getDefaultState());
            if (world.getBlockState(blockPos.up()).getMaterial().isReplaceable())
            {
                world.setBlockState(blockPos.up(), BlockFruitTreeLeavesTFCE.get(tree).getDefaultState().withProperty(BlockFruitTreeLeavesTFCE.HARVESTABLE, false));
            }
        }
    }
}