...
public class BlockCropDeadTFCE extends BlockBush
{
    ...
    private static final Map<ICrop, BlockCropDeadTFCE> MAP = new HashMap<>();

    public static BlockCropDeadTFCE get(ICrop crop)
    {
        return MAP.get(crop);
    }
    ...
    public BlockCropDeadTFCE(ICrop crop)
    {
    ...
    public int quantityDropped(IBlockState state, int fortune, Random random)
    {
        ...
        if (state.getValue(MATURE))
        {
            ...
            if (player != null)
            {
                ...
                if (skill != null)
                {
                    count += CropTFCE.getSkillSeedBonus(skill, RANDOM);
                    skill.add(0.04f);
                }
            }
        }
