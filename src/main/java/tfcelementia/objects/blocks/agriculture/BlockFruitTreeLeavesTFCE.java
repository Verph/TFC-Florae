...
public class BlockFruitTreeLeavesTFCE extends BlockLeaves
{
    public static final PropertyEnum<EnumLeafState> LEAF_STATE = PropertyEnum.create("state", BlockFruitTreeLeavesTFCE.EnumLeafState.class);
    public static final PropertyBool HARVESTABLE = PropertyBool.create("harvestable");
    private static final Map<IFruitTree, BlockFruitTreeLeavesTFCE> MAP = new HashMap<>();

    public static BlockFruitTreeLeavesTFCE get(IFruitTree tree)
    {
        return MAP.get(tree);
    }
    ...
    public BlockFruitTreeLeavesTFCE(IFruitTree tree)
    {
        this.tree = tree;
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            for (BlockPos p1 : paths)
            {
                for (EnumFacing face : EnumFacing.values())
                {
                    ...
                    if (state1.getBlock() == BlockFruitTreeTrunkTFCE.get(tree) || state1.getBlock() == BlockFruitTreeBranchTFCE.get(tree))
                        return;
