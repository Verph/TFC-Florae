...
public class WorldGenPodzol implements IWorldGenerator
{
    ...
    private void generatePodzol(World world, Random rng, BlockPos start)
    {
        ...
        if (rng.nextInt(ConfigTFC.WORLD.clayRarity) != 0 || start.getY() > WorldTypeTFC.SEALEVEL + 1) return;
        ...
        for (int x = -radius; x <= radius; x++)
        {
            for (int z = -radius; z <= radius; z++)
            {
                ...
                for (int y = -depth; y <= +depth; y++)
                {
                    ...
                    final IBlockState current = world.getBlockState(pos);if (BlocksTFCE.isGrass(current))
                    {
